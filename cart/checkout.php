		...
		if(isset($_POST['checkout'])){
			if($_SERVER['REQUEST_METHOD'] == "POST"){
				...
				if($_POST['pay'] != $_POST['hidden_total']){ ?>
					<div class="modal fade" id="totals" role="dialog" aria-hidden="true">
									<div class="modal-dialog">
										<div class="modal-content">
											...
											<div class="modal-footer">
												<a href="<?= BASE_URL ?>/index.php?p=checkout" class="btn btn-info">Ya</a>
											</div>
										</div>
									</div>
								</div>';
				...
				if(!$error){
					...
					foreach ($_SESSION["cart"] as $keys => $values) {
						...
						if(mysqli_query($conn, $order_detail)){
							unset($_SESSION['cart']); ?>
							<script>document.location = '<?= BASE_URL ?>/index.php?p=order'; </script>";
						<?php }
